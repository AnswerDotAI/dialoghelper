...
# %% ../nbs/00_core.ipynb #dd14352d
_file_edit_doc = """
This is a *file* editing function.

File editing standard parameters:

path: Path to the file to modify
log_changed: Add a note showing the diff?
...
def _file_edit(f, name=None):
    async def wrapper(path:str, *args, log_changed:bool=False, **kw):
        ...
        diff = '\n'.join(list(difflib.unified_diff(text.splitlines(), new_text.splitlines(), n=1, lineterm=''))[2:])
        if log_changed and diff: await add_msg(f"> Updated `{path}`\n\n```diff\n{diff}\n```")
        return diff or 'none: No changes.'
    res = splice_sig(wrapper, f, 'text')
    if name: res.__name__ = res.__qualname__ = name
...
file_insert_line = _file_edit(_insert_line, 'file_insert_line')
...
file_str_replace = _file_edit(_str_replace, 'file_str_replace')
...
file_strs_replace = _file_edit(_strs_replace, 'file_strs_replace')
...
file_replace_lines = _file_edit(_replace_lines, 'file_replace_lines')
